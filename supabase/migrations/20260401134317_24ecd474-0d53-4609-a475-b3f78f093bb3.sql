...
BEGIN
  ...
  -- Skip disconnect notifications for devices that were never connected (no number assigned)
  IF (OLD.number IS NULL OR OLD.number = '') AND (NEW.number IS NULL OR NEW.number = '') 
     AND NEW.status = ANY(_disconnected_statuses) THEN
    RETURN NEW;
  END IF;

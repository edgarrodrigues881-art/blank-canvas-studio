...
-- Update the OLD trigger function too (notify_device_disconnect) with same cooldown logic
...
BEGIN
  ...
  -- COOLDOWN: skip if same device+title notified in last 2 min
  ...
  -- MASS-EVENT: if 3+ same-title notifs in last 30s, send summary instead

...
BEGIN
  ...
  -- Auto-create Trial subscription for new users (if they don't already have one from legacy migration)
  SELECT EXISTS(
    SELECT 1 FROM public.subscriptions WHERE user_id = NEW.id
  ) INTO _has_subscription;

  IF NOT _has_subscription THEN
    INSERT INTO public.subscriptions (user_id, plan_name, max_instances, plan_price, started_at, expires_at)
    VALUES (NEW.id, 'Trial', 3, 0, now(), now() + interval '3 days');
  END IF;

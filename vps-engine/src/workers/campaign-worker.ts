...
function translateErrorMessage(msg: string): string {
  ...
  if (lower.includes("failed to get group members") || lower.includes("info query returned status 400")) return "Lead inválido — não é um número de telefone real (LID de comunidade)";
  if (lower.includes("bad-request") && lower.includes("query")) return "Lead não encontrado no WhatsApp";
  if (lower.includes("invalid number") || lower.includes("número inválido") || lower.includes("número nao encontrado") || lower.includes("número não encontrado")) return "Número inválido";
...
async function processOneCampaign(sb: any, campaign: any, isRunningRef: { value: boolean }) {
  ...
  while (isRunningRef.value) {
    ...
    if (!findDeviceById(currentDeviceId, allDevices)) {
      ...
      currentDeviceId = (allDevices[0] as any).id as string;
      instanceMsgCount = 0;
    ...
    if (heartbeatCounter % 10 === 0) {
      ...
      if (hbErr) logger.warn({ err: hbErr.message, campaignId }, "heartbeat_device_lock failed");
    }
...
// ══════════════════════════════════════════════════════════
// TICK: finds active campaigns and FIRES them as background tasks.
// Non-blocking: returns immediately so new campaigns are picked up fast.
// ══════════════════════════════════════════════════════════
...
export async function campaignWorkerTick(isRunningRef: { value: boolean }) {
  ...
  // Filter: skip campaigns already being processed
  ...
  log.info(`▶▶ Launching ${toProcess.length} campaigns (${campaigns!.length - eligible.length} already active)`);

  // Fire-and-forget: launch campaigns as background tasks (non-blocking)
  for (const campaign of toProcess) {
    const campaignId = campaign.id;

    // Mark as active immediately to prevent re-pickup on next tick
    activeCampaigns.add(campaignId);

    // Launch in background — do NOT await
    processOneCampaign(db, campaign, isRunningRef)
      .then(() => {
        log.info(`■ Campaign ${campaignId.slice(0, 8)} finished`);
      })
      .catch((err: any) => {
        log.error(`Campaign ${campaignId.slice(0, 8)} crashed: ${err.message}`);
      })
      .finally(() => {
        activeCampaigns.delete(campaignId);
      });

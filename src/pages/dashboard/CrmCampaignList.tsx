...
import { useCrmCampaigns, useDeleteCrmCampaign } from "@/hooks/useCrmCampaigns";
import { useCreateCrmTemplate } from "@/hooks/useCrmTemplates";
...
const CrmCampaignList = () => {
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      await Promise.all([
        supabase.from("crm_campaign_contacts").delete().in("campaign_id", ids),
        supabase.from("crm_campaigns").delete().in("id", ids),
      ]);
  ...
  const handleSaveAsTemplate = () => {
    ...
    createTemplate.mutate({
      ...
      message_type: savingCampaign.message_type || "texto",
      media_url: savingCampaign.media_url || undefined,
      buttons: Array.isArray(savingCampaign.buttons) ? savingCampaign.buttons : [],
    }, {
      onSuccess: () => {
        toast({ title: "Template CRM salvo!", description: `"${saveTemplateName.trim()}" salvo em Templates CRM.` });
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 5rem)" }}>
      <div className="shrink-0 space-y-5 pb-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-xl font-bold text-foreground tracking-tight">Campanhas CRM</h1>
            <p className="text-xs text-muted-foreground mt-0.5">Gerencie e acompanhe seus envios do CRM</p>
          </div>
          <div className="flex items-center gap-2">
            {campaigns.length > 0 && (
              <Button variant="ghost" size="sm" className="gap-1.5 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setClearAllOpen(true)}>
                <Trash2 className="w-3.5 h-3.5" /> Limpar todas
              </Button>
            )}
            <Button size="sm" className="gap-1.5 text-xs bg-primary hover:bg-primary/90 shadow-sm" onClick={() => navigate("/dashboard/crm-dispatches")}>
              <Plus className="w-3.5 h-3.5" /> Novo Disparo CRM
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 max-w-xs">
            ...
            <Input placeholder="Buscar..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs bg-card/50 border-border/30" />
          </div>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[130px] h-8 text-xs bg-card/50 border-border/30"><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain rounded-xl border border-border/30 bg-card/30" style={{ contain: "layout style", willChange: "scroll-position" }}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground/60">Nenhuma campanha CRM encontrada</p>
          </div>
        ) : (
          <div className="divide-y divide-border/20">
            {filtered.map((c) => {
              ...
              return (
                <div key={c.id} className="group grid items-center gap-4 px-4 py-3 cursor-pointer transition-colors duration-100 hover:bg-muted/20" style={{ gridTemplateColumns: "1fr 90px 100px 60px" }} onClick={() => navigate(`/dashboard/crm-campaign/${c.id}`)}>
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-muted-foreground tabular-nums whitespace-nowrap">{progress}% · {sent}/{total}</span>
                      {failed > 0 && <span className="text-[10px] text-destructive tabular-nums whitespace-nowrap">{failed} falhas</span>}
                    </div>
                  </div>
                  <div className="flex justify-center">
                    <Badge variant="outline" className={`text-[10px] font-semibold shrink-0 gap-1 px-2 py-0.5 h-5 ${cfg.color}`}>{cfg.icon}{cfg.label}</Badge>
                  </div>
                  <span className="text-[10px] text-muted-foreground tabular-nums text-right">{format(new Date(c.created_at), "dd/MM/yy HH:mm")}</span>
                  <div className="flex items-center gap-0.5 justify-end" onClick={(e) => e.stopPropagation()}>
                    {canSaveAsTemplate(c.status) && c.message_content && (
                      <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity duration-100 text-muted-foreground/40 hover:text-primary hover:bg-primary/10" title="Salvar como Template CRM" onClick={(e) => { e.stopPropagation(); setSavingCampaign(c); setSaveTemplateName(c.name || ""); setSaveTemplateOpen(true); }}>
                        <Save className="w-3.5 h-3.5" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity duration-100 text-muted-foreground/40 hover:text-destructive hover:bg-destructive/10" onClick={(e) => handleDelete(c.id, e)}>
      ...
      <AlertDialog open={clearAllOpen} onOpenChange={setClearAllOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Limpar todas as campanhas CRM?</AlertDialogTitle>
            <AlertDialogDescription>Isso excluirá permanentemente todas as {campaigns.length} campanhas CRM e seus contatos.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isClearingAll}>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleClearAll} disabled={isClearingAll} className="bg-destructive hover:bg-destructive/90">{isClearingAll ? "Excluindo..." : "Excluir todas"}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div>
              ...
              <Input value={saveTemplateName} onChange={(e) => setSaveTemplateName(e.target.value)} placeholder="Ex: Follow-up CRM" className="h-9" autoFocus onKeyDown={(e) => { if (e.key === "Enter") handleSaveAsTemplate(); }} />
            </div>
            <p className="text-[11px] text-muted-foreground">O template ficará disponível em <strong>Templates CRM</strong>.</p>
          </div>
          <DialogFooter>

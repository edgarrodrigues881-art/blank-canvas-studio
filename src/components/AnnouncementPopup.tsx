...
export function AnnouncementPopup({ announcement, onClose, onDismiss, isPreview }: AnnouncementPopupProps) {
  ...
  return createPortal(
    <AnimatePresence>
      {visible && (
        <div className="announcement-theme fixed inset-0 z-[9999] isolate" style={{ pointerEvents: "auto" }}>
          {/* Overlay */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
              ...
              <div className="relative overflow-hidden rounded-[20px] border border-border/40 bg-card shadow-[0_25px_80px_-15px_hsl(var(--primary)/0.25),0_0_0_1px_hsl(var(--border)/0.1)]">
                ...
                {announcement.allow_close && (
                  <button
                    ...
                    onClick={doClose}
                    className="absolute top-4 right-4 z-[60] flex h-8 w-8 items-center justify-center rounded-full border border-border/30 bg-muted/60 text-muted-foreground transition-all duration-200 hover:scale-110 hover:bg-muted hover:text-foreground"
                  >
                    <X size={14} />
                  </button>
                )}
